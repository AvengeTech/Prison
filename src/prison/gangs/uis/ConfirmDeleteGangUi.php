...
class ConfirmDeleteGangUi extends ModalWindow{
	...
	public function __construct(Player $player, $chance = false){
		...
		parent::__construct("Are you " . ($chance ? "SURE you're sure?" : "sure?"), "Are you sure you want to delete your gang? This action can NOT be undone.", "Delete Gang", "Go back");
	}
	...
	public function handle($response, Player $player) {
		...
		if($response){
			if($this->chance){
				$gang->selfDestruct(true);
				$gm->addLeftGang($player);
				return;
			}
			$player->showModal(new ConfirmDeleteGangUi($player, true));
			return;
		}
	}
