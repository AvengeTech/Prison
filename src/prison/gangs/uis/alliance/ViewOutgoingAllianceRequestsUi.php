...
class ViewOutgoingAllianceRequestsUi extends SimpleForm{

	public $invites = [];

	public function __construct(Player $player, $error = "") {
		/** @var PrisonPlayer $player */
		parent::__construct("Outgoing Alliance Requests", ($error == "" ? "" : TextFormat::RED . "Error: " . $error . TextFormat::WHITE . PHP_EOL . PHP_EOL) . "Select a request below for more information!");

		$gm = Prison::getInstance()->getGangs()->getGangManager();
		$gang = $gm->getPlayerGang($player);
		$aim = $gang->getAllianceInviteManager();

		foreach($aim->getOutgoingInvites() as $invite){
			$this->invites[] = $invite;
			$ally = $invite->getGang();
	...
	public function handle($response, Player $player) {
		...
		foreach($this->invites as $key => $invite){
			if($response == $key){
				if(!$invite->verify()){
					$player->showModal(new ViewOutgoingAllianceRequestsUi($player, "This request no longer exists!"));
					return;
				}

				$player->showModal(new OutgoingAllianceRequestUi($player, $invite));
				return;
			}
		}
