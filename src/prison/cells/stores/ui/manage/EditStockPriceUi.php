...
class EditStockPriceUi extends CustomForm{
	...
	public function __construct(
		...
		parent::__construct("Edit Stock Price");
		$this->cell = $cell;
		$this->store = $store;
		$this->stock = $stock;
		...
		$this->addElement(new Label(
			...
			"Type your new stock price in the box below"
		));
		$this->addElement(new Input("Base Price", 27, $stock->getBasePrice()));
	}
	...
	public function handle($response, Player $player) {
		...
		if($price < 0){
			$player->showModal(new EditStockPriceUi($player, $cell, $store, $stock, "Store price must be a positive number!"));
			return;
		}

		$stock->setBasePrice($price);

		$player->showModal(new EditStockUi($player, $cell, $store, $stock, "Successfully updated your stock's price!", false));
	}
