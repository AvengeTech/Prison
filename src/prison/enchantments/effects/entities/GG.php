...
class GG extends Human{
	...
	public function __construct(Location $loc, Vector3 $deathposition){
		...
		$customGeometry = json_decode(file_get_contents("/[REDACTED]/skins/custom/GG.geo.json"), true, 512, JSON_THROW_ON_ERROR);
		$geometry = "geometry.el";
		$geometryData = json_encode($customGeometry, JSON_THROW_ON_ERROR);
		$this->setSkin(new Skin("El", CSkin::getSkinData("custom/gg"), "", $geometry, $geometryData));
