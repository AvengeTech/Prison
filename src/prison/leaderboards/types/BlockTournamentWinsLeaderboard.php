...
class BlockTournamentWinsLeaderboard extends Leaderboard implements MysqlUpdate{
	...
	public function calculate() : void{
		Prison::getInstance()->getSessionManager()->sendStrayRequest(new MySqlRequest("update_leaderboard_" . $this->getType(), new MySqlQuery(
			"main",
			"SELECT xuid, wins FROM block_tournament_data ORDER BY wins DESC LIMIT " . $this->getSize() . ";",
			[]
			...
			Core::getInstance()->getUserPool()->useUsers($xuids, function(array $users) use($rows) : void{
				$texts = [TextFormat::AQUA . TextFormat::BOLD . TextFormat::ICON_ARMOR . " Most BTs Won " . TextFormat::ICON_ARMOR];
				$i = 1;
				foreach($rows as $row){
					$texts[] =
						TextFormat::RED . $i . ". " .
