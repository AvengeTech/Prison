...
class MineKillsLeaderboard extends Leaderboard implements MysqlUpdate{
	...
	public function calculate() : void{
		Prison::getInstance()->getSessionManager()->sendStrayRequest(new MySqlRequest("update_leaderboard_" . $this->getType(), new MySqlQuery(
			"main",
			"SELECT xuid, mine_kills FROM combat_stats ORDER BY mine_kills DESC LIMIT " . $this->getSize() . ";",
			[]
			...
			Core::getInstance()->getUserPool()->useUsers($xuids, function(array $users) use($rows) : void{
				$texts = [TextFormat::RED . TextFormat::BOLD . TextFormat::ICON_ARMOR . " Most Mine PvP Kills " . TextFormat::ICON_ARMOR];
				$i = 1;
				foreach($rows as $row){
					$texts[] =
						TextFormat::RED . $i . ". " .
						TextFormat::YELLOW . $users[$row["xuid"]]->getGamertag() . " " . TextFormat::GRAY . "- " .
						TextFormat::AQUA . number_format((int) $row["mine_kills"]);
					$i++;
				}
				$this->texts = $texts;
