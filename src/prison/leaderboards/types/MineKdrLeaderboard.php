...
class MineKdrLeaderboard extends Leaderboard implements MysqlUpdate{
	...
	public function calculate() : void{
		Prison::getInstance()->getSessionManager()->sendStrayRequest(new MySqlRequest("update_leaderboard_" . $this->getType(), new MySqlQuery(
			"main",
			"SELECT xuid, mine_kills / mine_deaths as kdr FROM combat_stats ORDER BY kdr DESC LIMIT " . $this->getSize() . ";",
			[]
			...
			Core::getInstance()->getUserPool()->useUsers($xuids, function(array $users) use($rows) : void{
				$texts = [TextFormat::RED . TextFormat::BOLD . TextFormat::ICON_ARMOR . " Highest Mine PvP KDR " . TextFormat::ICON_ARMOR];
				$i = 1;
				foreach($rows as $row){
					$texts[] =
						TextFormat::RED . $i . ". " .
